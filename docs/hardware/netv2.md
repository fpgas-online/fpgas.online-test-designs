...
The NeTV2 JTAG interface is directly wired to specific Raspberry Pi GPIO pins. OpenOCD uses the `bcm2835gpio` interface to bitbang these signals.
...
Configuration: `bcm2835gpio_jtag_nums 4 17 27 22` and `bcm2835gpio_srst_num 24`
...
### Direct Bitstream Load (volatile)

Using OpenOCD with the RPi GPIO JTAG interface:

```bash
openocd -f alphamax-rpi.cfg -c "init; pld load 0 top.bit; exit"
```

This loads the bitstream directly into the FPGA's configuration memory. The design is lost on power cycle.

### SPI Flash Programming (persistent)

Using OpenOCD with BSCAN_SPI bridge:

```bash
openocd -f alphamax-rpi.cfg \
  -c "init" \
  -c "jtagspi_init 0 bscan_spi_xc7a35t.bit" \
  -c "jtagspi_program top.bin 0x0" \
  -c "fpga_program" \
  -c "exit"
```
...
```bash
openFPGALoader --cable digilent_hs2 design.bit
```
...
| Programmer (OpenOCD) | `openocd_netv2_rpi.cfg` with BSCAN SPI |
| Programmer (openFPGALoader) | `digilent_hs2` cable |
