    ...
    "uart": {
        ...
        "boards": {
            ...
            "netv2": {"artifact": "uart-test-netv2/kosagi_netv2.bit",
                       "test_args": "--port /dev/serial0 --board netv2 --skip-banner",
                       "pre_test": "sudo systemctl stop 'serial-getty@*' 2>&1; which pm2 >/dev/null && pm2 stop all 2>&1; pgrep -f netv2-status && pkill -f netv2-status; sudo fuser -k /dev/serial0 2>&1; sudo chmod 666 /dev/serial0 2>&1; which pinctrl >/dev/null && sudo pinctrl set 14 a4 && sudo pinctrl set 15 a4; true"},
